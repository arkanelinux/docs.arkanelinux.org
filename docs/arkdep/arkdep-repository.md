...
## pkgs files
The .pkgs files are automatically generated by arkdep-build. They are an export of the package list in `gnome-shell 36.0` format. These files, if available, can be used by `arkdep get-update` to compute the package diff between versions or variants of the OS.
